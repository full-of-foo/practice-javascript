export default () => {
    ...
    console.log(`
        ...
        - Useful for enclosing scope, ie. just wrap everything you want made private.
    ...
    console.log(`
        -  Every function, while executing, has a reference to its current execution context, called 'this'.
           (Including its local variable stack, etc.)
        - Four rules for how the 'this' gets bound, and the all depend up the given callsite
          (where it gets executed). It doesn't matter whether there are classes/obj's involved.
          I just depends on these four callsite rules:
    `);
    console.log(`
        function foo(){
        ...
        new foo2('bar5').a; // 'bar5'
    `);
    console.log(`
        - 1. Default Binding: (L:9) when a bare fn or fn expression call is the callsite.
        -- if in strict mode, default 'this' to undefined. If not, 'this' gets the global object.
        - 2. Implicit Binding: (L:10/11) when our callset is object property reference to
             a function reference. (in JS everything is a reference to an object/function).
        -- the context/owning object becomes 'this'.
        - 3. Explicit Binding: (L:14) forcing a function call to use a particular object for the binding
             without putting a property function reference on the object.
        -- the passed object to 'call/apply' becomes 'this'.
        - 4. 'new' Binding: when calling a fn with 'new' in front of it, we construct a new object and set that
              new object as the 'this' for its execution context.
    `);
};
